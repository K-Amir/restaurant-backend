<div x-data="{ modelOpen: false }" class="h-full py-2">

    <button {{ $attributes }} class="button-add-reservation" @click="modelOpen =!modelOpen">
        {{ $slot }}
    </button>
    ...
    <div x-show="modelOpen" class="absolute w-full bottom-0 right-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
        ...
        <div class="container bg-dark-2 w-full rounded-[8px] mx-auto relative mh-fit">
            
            <div x-cloak class="inline-block w-full max-w-xl p-4 overflow-hidden text-left transition-all transform bg-white rounded-lg shadow-xl 2xl:max-w-2xl"
            >
                <div class="flex items-center justify-between space-x-4">
                    
                    <h4 class="titulo text-heading-2 text-light-3 text-center mb-2">Añadir reserva</h4>
                ...
                >
                    ...
                    <x-primary-button class="w-full text-center rounded-little py-3 mt-4" >
                        {{ __('Confirmar') }}
                    </x-primary-button>
                </form>
            </div>
